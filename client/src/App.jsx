      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "clamp(8px,2vw,14px)", width: "100%", maxWidth: 640 }}>
          {roles.map((role) => (
            <button key={role.path} onClick={() => navigate(role.path)} style={{
              ...
              borderRadius: "clamp(12px,3vw,18px)", border: `2px solid ${role.border}`,
              background: role.color, cursor: "pointer", minWidth: 0,
            }}>
              <div style={{ fontSize: "clamp(24px,5vw,36px)" }}>{role.emoji}</div>
              <div style={{ fontSize: "clamp(14px,3vw,20px)", fontWeight: 900, color: role.textColor ?? "#f6f7ff" }}>{role.label}</div>
              <div style={{ fontSize: "clamp(10px,2vw,12px)", color: "rgba(246,247,255,0.5)", textAlign: "center", lineHeight: 1.3 }}>{role.desc}</div>
            </button>
          ))}
        </div>

        {/* Editor card — smaller, below */}
        <button
        ...
        >
          ...
          <div style={{ textAlign: "left" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(246,247,255,0.7)" }}>Clue Editor</div>
            <div style={{ fontSize: 11, color: "rgba(246,247,255,0.35)" }}>Add, edit, or delete categories and clues</div>
          </div>
        </button>
