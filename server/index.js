...
io.on("connection", (socket) => {
  ...
  socket.on("host:selectClue", ({ colIndex, rowIndex }) => {
    ...
    state = {
      ...
      currentClue: {
        ...
        wrongPlayers: [],
        ...(clue.mediaUrl  ? { mediaUrl:  clue.mediaUrl  } : {}),
        ...(clue.mediaType ? { mediaType: clue.mediaType } : {}),
        ...(clue.publicId  ? { publicId:  clue.publicId  } : {}),
      },
      wager: null,
      buzz:  freshBuzz(),
    };
  ...
  socket.on("host:pauseVideo", () => {
    if (state.phase !== "clue" && state.phase !== "dailyDoubleClue") return;
    io.emit("video:pause");
  });

  socket.on("host:replayVideo", () => {
    if (state.phase !== "clue" && state.phase !== "dailyDoubleClue") return;
    if (!state.currentClue?.mediaUrl) return;
